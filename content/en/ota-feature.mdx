...
```mermaid
flowchart LR
  build[/"Compile the .bin file"/] --> upload[["Upload in the panel<br/>and activate"]]
  upload --> check["Device checks<br/>for an update"]
  check --> install["Download and write<br/>to flash"]
  install --> restart(["Restart with<br/>the new version"])
  ...
  classDef process fill:#15181d,stroke:#4b5563,color:#d1d5db,stroke-width:1.25px
  classDef outcome fill:#171625,stroke:#7167a8,color:#ede9fe,stroke-width:1.25px
  class build external
  class upload service
  class check,install process
  class restart outcome
